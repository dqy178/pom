...
<div class="main-content">
    <div class="max-w-4xl mx-auto glass-card p-6">
        <h2 class="text-2xl fw-bold mb-6">处方列表</h2>
            ...
            <tbody>
                <c:forEach items="${prescriptionList}" var="p">
                    <tr>
                        ...
                        <td>￥${p.totalAmount}</td>
                        <td>
                            <span class="px-2 py-1 rounded text-sm ${p.status == '已支付' ? 'bg-success-subtle text-success-emphasis' : 'bg-warning-subtle text-warning-emphasis'}">
                                ${p.status}
                            </span>
                        </td>
                        <td>${p.createTime}</td>
                        <td class="space-x-2">
                            <a href="${pageContext.request.contextPath}/user/prescriptionDetail?id=${p.id}" class="btn-primary px-3 py-1 rounded text-sm">详情</a>
                            <c:if test="${p.status != '已支付'}">
                                <button onclick="goToPay('PRESCRIPTION', '${p.id}')" class="btn-green px-3 py-1 rounded text-sm">支付</button>
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
